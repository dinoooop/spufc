...
const useGalleryStore = create((set) => ({
    ...
    index: async (data = {}) => {
        set({ loading: true });
        try {
            const response = await axios.get(`${config.api}/galleries`, {
                params: data,
    ...
    show: async (id) => {
        ...
        try {
            const response = await axios.get(`${config.api}/galleries/${id}`, config.header());
            set({
                loading: false,
                item: response.data,
            });
    ...
    store: async (data) => {
        set({ loading: true, success: '', error: '' });
        try {
            const response = await axios.post(`${config.api}/galleries`, data, config.header())
            set({
                loading: false,
                item: response.data,
            });
    ...
    update: async (data) => {
        ...
        try {
            set({ loading: true, success: '', error: '' });
            const response = await axios.put(`${config.api}/galleries/${id}`, data, config.header())
    ...
    destroy: async (data) => {
        ...
        set({ loading: true, success: '', error: '' });
        try {
            const response = await axios.delete(`${config.api}/galleries/${data._id}`, config.header())
        } catch (error) {
            set({
                loading: false,
                error: error.response ? error.response.data.message : 'An error occurred',
